...
export async function GET(request: Request) {
  ...
  // const profilePictures = await db.profilePictureUser.findMany({
  //   where: {
  //     userId: user.id
  //   },
  //   take: limit,
  ...
  //   }
  // });
  const profilePictures = await db.profilePicture.findMany({
    where: {
      gen: gen
    },
    select : {
  ...
  const count = await db.profilePicture.count({
    where: {
      gen: gen
    }
  });
